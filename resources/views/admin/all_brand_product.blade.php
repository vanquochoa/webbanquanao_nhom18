                        ...
                        @foreach ($all_brand_product as $item)
                            <tr>
                                ...
                                @if ($item->brand_status == 0)
                                    <td><a href="{{ URL::to('/unactive-brand-product/' . $item->brand_id) }}"><span
                                                class="fa fa-thumbs-down"> Ẩn</span></a></td>
                                @else
                                    <td>
                                        <a href="{{ URL::to('/active-brand-product/' . $item->brand_id) }}"><span
                                                class="fa fa-thumbs-up"> Hiện</span></a>
                                    </td>
                                @endif
                                <td>
                                    <a href="{{ URL::to('/edit-brand-product/' . $item->brand_id) }}"
                                         class="active"
                                        ui-toggle-class=""><i class="fa fa-pencil text-success text-active"></i></a>
                                    <a onclick="return confirm('Bạn có chắc muốn xóa?')" href="{{ URL::to('/delete-brand-product/' . $item->brand_id) }}"><i
